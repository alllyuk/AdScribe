...
class BLIP2LLMModel(BaseCaptionModel):
    ...
    def predict(self, image_bytes: bytes, features: str = None) -> str:
        ...
        full_prompt = f"""
            На изображении показан предмет одежды (описание на английском языке, нужно перевести его и использовать): {visual_desc}.
            {"Характеристики этого предмета:" + features.strip() if features else ''}
            Сформулируй привлекательное и лаконичное описание для онлайн-объявления на основе полученной информации.
            В спорной ситуации между описанием на английском и характеристиками предмета выбирай информацию из характеристик.
            ...
            Не форматируй результат никакими спецсимволами. Также не пиши в начале результата генерации ничего типа "твой результат", "итоговое описание" или другого.
            """
